...
public class RunIterator implements Iterator {
	...
	private DList list;    //so that RunIterator knows the methods and obj...in DList

	/**
	 *  RunIterator() constructs a new iterator starting with a specified run.
	 * @param runLengthEncoding 
	 *
	...
	public int[] next() {
		if(hasNext()) {
			...
			arr=new int[4];
			arr[0]=pointer.runLengths;
			arr[1]=pointer.color[0];
			arr[2]=pointer.color[1];
			arr[3]=pointer.color[2];
